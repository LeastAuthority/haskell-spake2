...
tests = testSpec "Integration" $
  ...
  where
    ...
    exchangeWithPython protocol password args = do
        ...
        let testScript =
                (Process.proc "python" (scriptExe : ("--code=" <> toS password) : args))
        ...
        Process.withCreateProcess testScript $
            \(Just stdin) (Just stdout) _stderr ph -> do
                ...
                ( do
                        Right sessionKey <- Spake2.spake2Exchange protocol (Spake2.makePassword password) (send stdin) (receive stdout)
                        theirSpakeKey <- ByteString.hGetLine stdout
                        theirSpakeKey `shouldBe` convertToBase Base16 sessionKey
                    )
                    `finally` Process.waitForProcess ph
